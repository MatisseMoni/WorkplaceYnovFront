...
function GroupesList({ access = 'all' }) {
	...
	const retrieveGroupes = async () => {
		try {
			...
			if (access === 'owner') {
				console.log('owner');
				groupesTmp = groupesTmp.filter(
					(groupe) => groupe.owner === `/api/users/${currentUser.id}`
				);
				dispatch(setGroupesOwned(groupesTmp));
			} else if (access === 'member') {
				console.log('member');
				groupesTmp = await Promise.all(
					groupesTmp.map(async (groupe) => {
				...
				groupesTmp = groupesTmp.filter((groupe) =>
					groupe.members.includes(`/api/users/${currentUser.id}`)
				);
	...
	const retrieveMembers = async (groupe) => {
		try {
			let response = await axios.get(
				`${process.env.REACT_APP_YOUR_API_URL}/api/groups/${groupe.id}/members`
			);
